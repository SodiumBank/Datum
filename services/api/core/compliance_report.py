...
from services.api.core.profile_stack import resolve_profile_stack
...
def generate_approvals_trail(plan: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if plan.get("status") != "draft":
        # Note: We don't store explicit "submitted" timestamp, infer from status
        events.append({
            "event_type": "submitted",
            "status": "submitted",
            "timestamp": plan.get("edited_at"),  # Approximation
        })
